...
from detector.yolo_engine import run_yolo_tile
...
def process_image_task(job_id: str, b64_str: str):
    ...
    try:
        ...
        all_raw = []
        # In a real app we use ThreadPoolExecutor here. Simplifying for orchestration module.
        for t in tiles:
            patch_dets = run_yolo_tile(t[0])
            for d in patch_dets:
                d["box"][0] += t[1]
                d["box"][1] += t[2]
                d["box"][2] += t[1]
        ...
        for fd in final_dets:
            ...
            detections.append(AIBox(
                ...
                text="",
                redacted=(fd["label"] in ["pii_text", "face", "nsfw"])
            ))
...
"""
    with open(os.path.join(base, "backend", "main.py"), "w", encoding="utf-8") as f:
        f.write(backend_main)
        
    # Overwrite root main.py
    root_main = """import uvicorn
